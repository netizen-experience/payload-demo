...
- **Lambda has a hard 6MB response payload limit for synchronous invocations, and `/api/media/file/<filename>` buffers the whole file into the response** — any original upload over ~6MB 404s... actually 502s (`RequestEntityTooLarge`, `Exceeded maximum allowed payload size (6291556 bytes)`) when fetched directly. Confirmed this affects multiple pre-existing large originals (7-8MB dish photos), not something introduced by the restore work. In practice this doesn't affect the rendered site — Next.js Image always requests a resized `sizes.*` variant (all comfortably under 6MB), never the raw original — but direct links to an original file that's large enough will break. No fix implemented; would need either a streaming response mode for that route or a size cap on original uploads.
